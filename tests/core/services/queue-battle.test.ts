...
describe('QueueBattleService', () => {
  ...
  describe('executeRound', () => {
    ...
    test('should return unchanged state if queue is incomplete', () => {
      const { battle } = createTestBattle();
      const rng = makePRNG(12345);

      // Only queue 2 actions (need 4 for all alive units)
      let state = battle;
      state = queueActionUnwrap(state, 'unit1', STRIKE.id, ['enemy1'], STRIKE);
      state = queueActionUnwrap(state, 'unit2', STRIKE.id, ['enemy1'], STRIKE);

      // executeRound gracefully handles incomplete queue (no throw, returns unchanged)
      const result = executeRound(state, rng);
      expect(result.events).toEqual([]);
      expect(result.state.phase).toBe('planning'); // Still in planning phase
    });
